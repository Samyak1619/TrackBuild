...
// Function to generate a unique user ID with role-based prefixes
function generateUniqueUserID($conn, $role) {
    $prefix = '';
    switch ($role) {
        case 'admin':
            ...
            break;
        case 'user':
            $prefix = 'USR'; // Prefix for user
            break;
        case 'siteManager':
            $prefix = 'SMG'; // Prefix for site manager
            break;
        default:
            $prefix = 'GEN'; // Generic prefix for unexpected roles
    }
    ...
    if ($stmt->num_rows > 0) {
        ...
        return generateUniqueUserID($conn, $role); // Recursive call to ensure uniqueness
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name) || empty($email) || empty($phone) || empty($constructionType) || empty($password) || empty($role)) {
    ...
    $allowedRoles = ['user', 'admin', 'siteManager'];
    ...
    $userid = generateUniqueUserID($conn, $role);

    // Insert data into the database
    $sql = "INSERT INTO user_registration (userid, name, email, phone, construction_type, role, password, latitude, longitude)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

    // Prepare and bind
    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("sssssssss", $userid, $name, $email, $phone, $constructionType, $role, $password, $latitude, $longitude);

        if ($stmt->execute()) {
            // Pass user ID to the next page via session
            session_start();
